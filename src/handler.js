...
const getAllBooksHandler = (request, h) => {
  ...
  if (reading) {
    const filterReadingBooks = books.filter((b) => Number(b.reading) === Number(reading));
    const response = h.response({
      status: 'success',
      data: {
  ...
  if (name) {
    const filteredBooksName = books.filter((book) => {
      const names = new RegExp(name, 'gi');
      return names.test(book.name);
    });
    const response = h.response({
      status: 'success',
      data: {
  ...
  if (finished) {
    const filterBooksFinished = books.filter((b) => Number(b.finished) === Number(finished));
    const response = h.response({
      status: 'success',
      data: {
...
const deletBooksHandler = (request, h) => {
  const { id } = request.params;
  const index = books.findIndex((b) => b.id === id);
  if (index !== -1) {
